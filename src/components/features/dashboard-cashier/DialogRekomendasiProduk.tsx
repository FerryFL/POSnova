...
import type { Produk, RekomendasiProdukAI } from "~/utils/api";

interface DialogRekomendasiProdukProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    rekomendasi?: RekomendasiProdukAI
...
const DialogRekomendasiProduk = (props: DialogRekomendasiProdukProps) => {
    ...
    // console.log("ini rekom: ", rekomendasi)
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[90vw]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                    {rekomendasi && rekomendasi.recommendations.length > 0 ? (
                        rekomendasi.recommendations.map((item) => (
